...
function bling(){
    worm.style.fontSize = '28px'; //not in css so should be a string
...
recipe.addEventListener('click', add);

function add(){
    var block = document.createElement('li');
    block.className = 'mix';
    block.innerHTML = missing;
    ingredient.appendChild(block);
}
...
function changeImg(){
    if(puggy.src === 'https://bit.ly/2RhOvm4'){
        puggy.src = puggy.src = 'https://media.giphy.com/media/12pJ8OxSWwO86Y/giphy.gif';
    }else{
...
menu.addEventListener('click', dessertCheck);
...
function showProfile(){
    ...
    name.innerHTML = 'Name: ' + grimace.name;
    bio.appendChild(name);

    var sign = document.createElement('li');
    sign.innerHTML = 'Sign: ' + grimace.sign;
...
fortune.addEventListener('click', desc);

function desc(){
    var random = Math.floor(Math.random()*messages.length);
    showFortune.innerHTML = messages[random];
}
...
secret.addEventListener('keyup', makeCaps);
